...
import { deleteLikeById } from 'apiSdk/likes';
...
function RecipeViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<RecipeInterface>(
    ...
    () =>
      getRecipeById(id, {
        relations: ['account', 'like', 'review'],
      }),
  );

  const likeHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('recipe', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/recipes/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('like', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/likes/create?recipe_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.like?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/likes/view/${record.id}`)} key={record.id}>
                            <Td>
                              {hasAccess('like', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink passHref href={`/likes/edit/${record.id}`}>
                              ...
                              {hasAccess('like', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={() => likeHandleDelete(record.id)}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
                                  icon={<FiTrash />}
              ...
              {hasAccess('review', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/reviews/create?recipe_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                          ...
                          >
                            ...
                            <Td>
                              ...
                              {hasAccess('review', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={() => reviewHandleDelete(record.id)}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
                                  icon={<FiTrash />}
